...
lein with-profile +release,+gen set-version ${version} :previous-version ${previous_version} \
...
&& lein with-profile +doc,+gen doc \
&& lein with-profile +release,+gen set-version ${next_version} \
&& git add project.clj \
